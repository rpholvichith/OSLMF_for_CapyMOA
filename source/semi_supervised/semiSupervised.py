...
def PairDotsDistance(Dots, NDim, numOfDots):
    ...
    for i in range(0, numOfDots - 1):
        ...
        matIndex = matIndex + 1
    Distance = np.array(list_of_groups(DistanceMat, 3)) # Returns a O(numOfDots²) x 3 matrix: 1) 1st dot. 2) 2nd dot. 3) eulerian distance
    return Distance
...
def DensityPeaks(xi, precent):
    ...
    xiT = xi.T
    xx = PairDotsDistance(xiT, colN, rowN)  # shape: O(rowN²) x 3
    xxT = xx
    ND = np.max(xxT[:, 1]) + 1              # ND: should be rowN
    NL = np.max(xxT[:, 0]) + 1              # NL: should be rowN-1
    ...
    # dc: distance (float) corresponding to the '5' percent smallest distances in the PairDotsDistance vector
    dc = sda[int(position)]
    ...
    rho_sorted = rho_list               # rho (array) sorted in decreasing order
    ordrho = np.argsort(-rho,axis=1)    # rho (array) corresponding indices

    delta = np.zeros((int(ND)))
    delta[ordrho[0,0]] = -1 # - 1] = -1.        #???
    delta = delta.reshape(-1, delta.shape[0])

    nneigh = np.zeros((int(ND)))
    nneigh[ordrho[0,0]] = 0 # - 1] = 0
    ...
    for ii in range(1, int(ND)):
        ...
        for jj in range(0, ii - 1):
            if (dist[ordrho[0, ii], ordrho[0, jj]] < delta[0, ordrho[0, ii]]):
                ...
                nneigh[0, ordrho[0,ii]] = ordrho[0, jj]     # for i-th point with the largest density, its neighbor is one with greater density and closest to it
    ...
    # makes the smallest index the root?
    b = np.argmin(nneigh)
    nneigh[b,0] = b
...
def SSC_DensityPeaks_SVC(train, label_train, t, nneigh, clf): # , test, label_test #, clf, n, decay_choice, contribute_error_rate
    ...
    while (len(struct) > 0):
        ...
        try:
            ...
            print("Could not fit or predict during downward label propagation.")
            continue
    ...
    while (len(struct) > 0):
        ...
        try :
            ...
            print("Could not fit or predict during upward label propagation.")
            continue
    ...
    try:
        ...
        print("Could not fit all data in SSC_DensityPeaks_SVC.")
...
def SSC_DensityPeaks_SVC_ensemble(train_1, label_train_1, train_2, label_train_2, t, nneigh_x, nneigh_z, clf1, clf2): # , test, label_test #, clf, n, decay_choice, contribute_error_rate
    ...
    data_x = []
    label_data_x = []
    label_U_x = []      # label of Unobserved data
    ...
    t_U_x = np.setdiff1d(diff_array_x, t-1) # Unobserved label indices?
    if len(t_U_x) != 0:
    ...
    while (len(struct_x) > 0):  # while we can propagate labels
        ...
        # add newly labeled points to data
        data_TR_x = data_x
        ...
        try:
            # fit and predict on data points (state at start of iteration)
            classifier_x.fit(data_TR_x, lable_TR_x)
            label_data_x = classifier_x.predict(data_x)
        except:
            print("[ensemble] Could not fit or predict during 1st downward label propagation.")
            continue
    ## Labels have been totally propagated (downward) through convex tree structure
    ...
    while (len(struct_x) > 0):
        data_neigh_x = list(data_neigh_x)
        k_x = 0
        # get parents of labeled points (number_neigh_x)
        for i in range(0, len(struct_x), 1):
        ...
        # get unlabeled parents
        struct_x = np.setdiff1d(data_neigh_x, struct_record_x)
        length_struct_x = len(struct_x)

        # label the parents (add to struct_record_x)
        ...
        try:
            ...
            print("[ensemble] Could not fit or predict during 1st upward label propagation.")
            continue
    ## Labels have been totally propagated (upward) through convex tree structure
    ...
    while (len(struct_z) > 0):
        ...
        try:
            ...
            print("[ensemble] Could not fit or predict during 2nd downward label propagation.")
            continue
    ...
    while (len(struct_z) > 0):
        ...
        try:
            classifier_z.fit(data_TR_z, label_TR_z)
            label_data_z = classifier_z.predict(data_z)
        except:
            print("[ensemble] Could not fit or predict during 2nd upward label propagation.")
            continue

    '''
        Ensemble Learning
    '''
    label_data_x = label_data_x.astype('int')   # was predicted in the label propagation loops
    ...
    for n in range(length):
        ...
        try:
            ...
            predict_label_train_2 = predict_label_train_2[0, 1]
        except IndexError:
            predict_label_train_2 = np.random.randint(0, 2, 1)
        except :
            predict_label_train_2 = np.random.randint(0, 2, 1)

        predict_label_train = lamda * predict_label_train_1 + (1.0 - lamda) * predict_label_train_2 #sigmoid(lamda * predict_label_train_1 + (1.0 - lamda) * predict_label_train_2)
